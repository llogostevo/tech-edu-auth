...
export default function CommentForm({ slug, email, first_name, user_id, last_name }: { slug: string; email:string, user_id: string, first_name: string, last_name: string }) {
    // `/api/comments/${slug}`
    ...
    async function handleFormSubmit(event: React.FormEvent<HTMLFormElement>) {
        ...
        const { data, error } = await supabase
            .from('comments')
            .insert([
                { content: comment , blog_id: `${slug}`, user_profile_id: `${user_id}` },
            ])
            .select()
